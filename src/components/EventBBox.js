...
class EventBBox extends React.Component {
    constructor(props) {
      super(props);
      this.state = {
    ...
    init(){
      ...
      imgObj.src = item.origin_picture
        ...
        imgObj.onload = function(event){
          ...
          console.log('origin_box',_bbox_data)
          // console.log(typeof(_bbox_data))
          //[[Decimal('0'), Decimal('0.27'), Decimal('0.74'), Decimal('0.31'), Decimal('0.39')]]
          
          // _bbox_data = _bbox_data.slice(1, _bbox_data.length-1)
  ...
  async save(){
    ...
    // const apiUrl = '/save_event_result'+'/'+ docid;
    // console.log(apiUrl)
    var result = "";
    //payload = { "hello" : "test"}
    await API.post('backend','/verifyEvent',{ body: payload, Headers: HEADERS }).then(response => {
...
onHelmetChange(e){
  ...
  renderBBox(){
    ...
    render(){
      ...
      // img_show_w:300,
      // img_show_h:400,
      const c_id = 'bbox'+this.props.data.TimeStamp
      ...
      return (
        <Container headingVariant="h4" title={this.props.data.id}>
...
export default connect(mapStateToProps,MapDispatchTpProps)(withTranslation()(EventBBox));
