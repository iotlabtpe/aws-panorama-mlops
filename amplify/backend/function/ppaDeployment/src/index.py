...
def post(event, account_id):
    ...
    for camera in cameras:
        ...
        override_camera_template["nodeGraphOverrides"]["nodes"].append(
            {
                "name": "{}".format(camera),
                "interface": "{acc_id}::{name}.{name}".format(
                    acc_id=account_id, name=camera
                ),
                "overridable": True,
        ...
        override_camera_template["nodeGraphOverrides"]["nodeOverrides"][0][
            "with"
        ].append({"name": "{}".format(camera)})

    # Override env node into Panorama Application
    override_camera_template["nodeGraphOverrides"]["nodes"].append(
        {"name": "my_env", "interface": "string", "value": env_p}
    )
    override_camera_template["nodeGraphOverrides"]["nodeOverrides"].append(
        {"replace": "environment_variable", "with": [{"name": "my_env"}]}
    )
    ...
    try:
        resp = pano_client.create_application_instance(
            ...
            ManifestOverridesPayload={
                "PayloadData": json.dumps(override_camera_template)
            },
