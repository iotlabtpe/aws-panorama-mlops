...
def handler(event, context):
    ...
    s3_client.download_file(RETRAIN_NAME, model_name + "/output/model.tar.gz", "/tmp/model.tar.gz")
    ...
    s3_client.upload_file("/tmp/metrics.json", RETRAIN_NAME, model_name + "/metrics.json")
